...
/**
 * @brief MUSA kernel to compute partial trace sums per block
 */
template <typename T>
...
T trace(const std::vector<T>& h_input, size_t rows, size_t cols) {
    ...
    musaMalloc(&d_input, input_size * sizeof(T));
    musaMalloc(&d_block_results, numBlocks * sizeof(T));
    musaMemcpy(d_input, h_input.data(), input_size * sizeof(T), musaMemcpyHostToDevice);
    ...
    // ========================================================================
    if (numBlocks == 1) {
        musaMemcpy(&result, d_block_results, sizeof(T), musaMemcpyDeviceToHost);
    }
    // ========================================================================
    ...
    // ========================================================================
        ...
        musaMemcpy(h_block_results.data(), d_block_results, 
                   numBlocks * sizeof(T), musaMemcpyDeviceToHost);
    ...
    // ========================================================================
        ...
        T* d_output = nullptr;
        musaMalloc(&d_output, sizeof(T));
        
        int reduceBlockSize = 256;
        reduceBlockResults<<<1, reduceBlockSize, reduceBlockSize * sizeof(T)>>>(
            d_block_results, numBlocks, d_output);
        
        musaMemcpy(&result, d_output, sizeof(T), musaMemcpyDeviceToHost);
        musaFree(d_output);
    }
    
    musaFree(d_input);
    musaFree(d_block_results);
...
/**
 * @brief Flash Attention kernel with numerical stability for MUSA
 * 
 * Features:
...
void flashAttention(const std::vector<T>& h_q, const std::vector<T>& h_k,
    ...
    musaMalloc(&d_k, k_size * sizeof(T));
    musaMalloc(&d_v, v_size * sizeof(T));
    musaMalloc(&d_o, o_size * sizeof(T));
    
    musaMemcpy(d_q, h_q.data(), q_size * sizeof(T), musaMemcpyHostToDevice);
    musaMemcpy(d_k, h_k.data(), k_size * sizeof(T), musaMemcpyHostToDevice);
    musaMemcpy(d_v, h_v.data(), v_size * sizeof(T), musaMemcpyHostToDevice);
    ...
    if (err != musaSuccess) {
        printf("[%s] MUSA Error: %s\n", PLATFORM_NAME, musaGetErrorString(err));
    }
